...
(function() {
	...
	function buildInfo(data, indx) {
		...
		if (boostPFSThemeConfig.custom.show_product_info) {
            ...
            html += '<div class="ProductItem__Header">';
			html += '<h2 class="ProductItem__Title Heading">';
			html += '<a href="{{itemUrl}}">' + data.title + '</a>';
			html += '</h2>';
            html += buildPrice(data);
            html += '</div>';
            html += '<div class="ProductItem__Impact">'
            if (Utils.getProductMetafield(data, "impact", "icon")) {
              html += '<img src="'+Utils.getProductMetafield(data, "impact", "icon")+'" alt="'+Utils.getProductMetafield(data, "impact", "cause")+' icon" />'
            }
            html += Utils.getProductMetafield(data, "impact", "grid_text")
            html += '</div>';
			html += '{{itemReviews}}';
