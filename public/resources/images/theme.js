...
const moq = document.getElementById('moq-blank');
if (moq != null) {
  const moqNumber = moq.innerText;
  var moqBlank = moqNumber;
  console.log(moqNumber);
} else {
  var moqBlank = 1;
}
  ...
  var ProductVariants = function () {
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _decreaseQuantity(event, target) {
        target.nextElementSibling.value = Math.max(parseInt(target.nextElementSibling.value) - 1, moqBlank);
      }
      ...
      value: function _validateQuantity(event, target) {
        target.value = Math.max(parseInt(target.value) || 1, moqBlank);
      }
    }]);
